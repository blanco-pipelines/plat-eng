...
2. Navigate to the root folder and run ls. Use the ls command on the folders you see. Different operating systems may have different locations for some folders.
   ...
   graph LR
       ROOT[/ / (root)]:::root
       ROOT --> BIN[/bin<br/>(user binaries)]:::dir
       ROOT --> USR[/usr<br/>(shared user data)]:::dir
       ROOT --> VAR[/var<br/>(variable data)]:::dir
       ROOT --> ETC[/etc<br/>(system configs)]:::dir
       ROOT --> HOME[/home<br/>(user directories)]:::dir
       ROOT --> TMP[/tmp<br/>(temporary files)]:::dir
   
       VAR --> LOG[/var/log<br/>(log files)]:::desc
       LOG --> APPLOGS[application logs]:::desc
   
       BIN --> LS[ls]:::file
       BIN --> CD[cd]:::file
   
       ETC --> SUDO[sudoers.d]:::desc
       ETC --> LOGROTATE[logrotate.d]:::desc
       ...
       classDef desc fill:#555,stroke:#ccc,stroke-width:1px,color:#fff;
