...
This playbook:
1. Sets up NFS
2. Creates restore task (S3 → NFS)
3. Executes restore and waits for completion
4. Creates backup task (NFS → S3) with schedule
...
View DataSync execution history in the **AWS Console** under DataSync → Tasks, or use CLI:
...
- ✅ AWS DataSync automated sync to S3 primary bucket (every minute)
...
1. ✅ Sets up NFS
2. ✅ Creates DataSync restore task (S3 → NFS)
3. ✅ Executes restore and waits for completion
4. ✅ Creates backup task (NFS → S3) with schedule
...
echo "🎯 Recovery Time Objective (RTO): ${RTO_DURATION} seconds ($((RTO_DURATION / 60)) minutes)"
echo "🎯 Recovery Point Objective (RPO): ~60 seconds (last DataSync sync)"
```
