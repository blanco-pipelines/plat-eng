...
- Completed Lab 3.1 (GitOps with Argo CD)
...
### Step 4: (Optional) Launch Rollouts Dashboard

The dashboard provides a visual interface for rollouts:
...
> You can run this in the background and access it throughout the lab.
...
git commit -m "Convert to blue/green rollout"
git push origin main
```

### Step 4: Update Argo CD Application Path

The Argo CD application is still looking for the old path. Since we're in the same directory, Argo CD will automatically detect the new `rollout.yaml` file. Just wait for it to sync.

### Step 5: Wait for Argo CD to Sync
...
@app.route('/')
def index():
    return 'Hello Argo CD v2.0 - Blue/Green Deployment!'
...
```bash
git add main.py
git commit -m "Version 2.0 - Blue/Green test"
git tag v2.0.0
git push origin main
...
This triggers:
1. GitHub Actions builds `v2.0.0` image
...
2. Review the PR updating the image to `v2.0.0`
3. Merge the PR

### Step 4: Watch the Blue/Green Deployment
...
### Step 5: Test the Preview Environment
...
# Should return: Hello Argo CD v2.0 - Blue/Green Deployment!
```
...
# Should still return: Hello Argo CD v1.0!
```
...
2. Traffic instantly cuts over to v2.0.0
...
curl http://<ACTIVE-SERVICE-URL>:8080
# Should now return: Hello Argo CD v2.0 - Blue/Green Deployment!
```

### Step 8: Check Argo Rollouts Dashboard

If you're running the dashboard (`kubectl argo rollouts dashboard`), open `http://localhost:3100` and view the visual representation of your rollout.
...
@app.route('/')
def index():
    return 'v3.0.0 - BROKEN VERSION!'  # Pretend this is broken
...
```bash
git add main.py
git commit -m "Version 3.0 - broken"
git tag v3.0.0
git push origin main
...
### Bonus 2: Pre-Promotion Analysis

Add a health check before promotion:
...
strategy:
  blueGreen:
    ...
    prePromotionAnalysis:
      ...
      args:
      - name: service-name
        value: example-application-preview
```

### Bonus 3: Post-Promotion Analysis

Monitor metrics after promotion and auto-rollback if issues detected:
...
(Requires Prometheus or other metrics provider)
...
**If eksctl fails to delete everything:**
```bash
# You can force delete via AWS Console:
