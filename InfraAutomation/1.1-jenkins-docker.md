...
**Key Insight:** 
The `sh` step runs commands in a shell (`/bin/sh`). When you use Jenkins plugins (like Docker Pipeline's `docker.build()`), they're actually **abstractions that execute shell commands behind the scenes**. The plugins provide a cleaner Groovy DSL, but ultimately they're running `docker build`, `docker push`, etc. in a shell.
