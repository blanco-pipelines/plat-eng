# Lab 4.2: GitOps with Kubernetes and Argo CD
...
jobs:
  build-and-push:
    ...
    steps:
      ...
      # TODO: Add Docker login step here
      # Hint: Use the docker/login-action@v3
      # Use the secrets you created: REGISTRY_USER and REGISTRY_TOKEN

      # TODO: Add Docker build and push step here
      # Hint: Use docker/build-push-action@v5
      # Set context to current directory
      # Push to: docker.io/${{ secrets.REGISTRY_USER }}/example-application:${{ env.TAG_NAME }}
...
Complete the `build-and-push` job by adding:
1. **Docker login step** - Authenticate to Docker Hub using your secrets
2. **Build and push step** - Build the Docker image and push it to your Docker Hub repository

**Hints:**
- Look at the GitHub Actions Marketplace for `docker/login-action` and `docker/build-push-action`
- Use the `REGISTRY_USER` and `REGISTRY_TOKEN` secrets you configured earlier
- The image should be tagged with the Git tag name (already extracted as `TAG_NAME`)
- Don't forget to set `push: true`!

> If you get stuck, the complete solution is at the bottom of this lab.
...
Clone your `example-environment` repository locally and create the following structure:
...
Create `applications/example-application/deployment.yaml` with the following content:
...
Create `.github/workflows/update-deployment.yaml` with the following content:
...
Commit and push both files to your `example-environment` repository:

```bash
git add applications/ .github/
...
Your application is deployed, but does it actually work? Time to find out!

**Your task:** Access your Flask application in a web browser. 

**Hints:**
- Remember how you got the Argo CD UI URL? (Step 6 in Part 7)
- Your application also has a LoadBalancer service
- The app runs on port 8080
- You should see a "Hello Argo CD" message

If you get stuck, think about what Kubernetes resource exposes applications to the outside world.
...
### Step 1: Clone and Tag the Application Repository

Clone your `example-application` repository locally:
...
Create and push the `v1.0.0` tag to trigger the build pipeline:
...
> This tag triggers the GitHub Actions workflow that builds and pushes the Docker image.
...
Don't just trust Argo CD - actually test your application!

**Challenge:** Access your application and confirm you see the updated message from v1.0.0.

**Hint:** You already know how to get the LoadBalancer URL from earlier in the lab.
...
## Appendix: Solution to Part 5 Challenge

If you got stuck completing the `release.yaml` workflow in Part 5, here's the complete solution:

<details>
<summary>Click to reveal the complete release.yaml</summary>
...
1. **Docker Login**: Uses `docker/login-action@v3` with your Docker Hub credentials from secrets
2. **Build and Push**: Uses `docker/build-push-action@v5` to build from current directory and push to Docker Hub
3. **Tagging**: Uses the Git tag (extracted as `TAG_NAME`) to tag the Docker image
4. **Dependencies**: The `trigger-deployment-update` job waits for `build-and-push` to complete via `needs`
